...
seurat_object <- ScaleData(seurat_object, vars.to.regress = c("orig.ident", "percent_mt", "percent_rb", "batch"))

er_expression <- seurat_object %>%
    dplyr::mutate(protected = factor(protected)) %>%
    FetchData(vars = c("protected", er_genes_of_interest), layer = "scale.data") |>
    pivot_longer(
        cols = -protected,
        names_to = "gene",
