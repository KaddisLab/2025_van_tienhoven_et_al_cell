...
de_markers <- FindMarkers(beta.bulk,
    ident.1 = "TT", ident.2 = "AA", slot = "counts", test.use = "DESeq2",
    verbose = F
)
...
ggplot(de_markers, aes(x = avg_log2FC, y = -log10(p_val))) +
    ...
    labs(
        ...
        subtitle = "Comparison protected (AA ←) vs susceptible (TT →) cells",
    ) +
    geom_text_repel(
...
```{r }
de_markers <- FindMarkers(beta.bulk,
    ident.1 = "TT", ident.2 = "AA", slot = "counts", test.use = "wilcox",
    verbose = F
)
