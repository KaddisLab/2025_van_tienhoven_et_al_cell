...
bulk <- AggregateExpression(
    ...
    return.seurat = TRUE,
    assays = "RNA",
    group.by = c("cell_type", "orig.ident", "protected")
) |>
    separate(.cell, into = c("cell_type", "orig.ident", "protected"), sep = "_", remove = FALSE)
...
de_markers <- FindMarkers(beta.bulk,
    ident.1 = "TRUE", ident.2 = "FALSE", test.use = "DESeq2", verbose = FALSE
)
...
ggplot(de_markers, aes(x = avg_log2FC, y = -log10(p_val))) +
    ...
    labs(
        ...
        subtitle = "Comparison susceptible (GG ←) vs Protected (GA →) cells",
    ) +
    geom_text_repel(
...
```{r }
de_markers <- FindMarkers(beta.bulk,
    ident.1 = "TRUE", ident.2 = "FALSE", test.use = "wilcox", verbose = F
)
