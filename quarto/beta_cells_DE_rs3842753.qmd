...
de_markers <- FindMarkers(beta.bulk,
    ident.1 = "TG", ident.2 = "TT", test.use = "DESeq2", verbose = FALSE
)
...
ggplot(de_markers, aes(x = avg_log2FC, y = -log10(p_val))) +
    ...
    labs(
        ...
        subtitle = "Comparison protected (TT ←) vs susceptible (TG →) cells",
    ) +
    geom_text_repel(
...
```{r }
de_markers <- FindMarkers(beta.bulk,
    ident.1 = "TG", ident.2 = "TT", test.use = "wilcox", verbose = F
)
