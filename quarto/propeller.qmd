...
propeller(clusters = cell_info$tosti_cell_type, sample = cell_info$orig.ident,
...
plot_protected<-plotCellTypeProps(
    clusters = protected$tosti_cell_type,
...
plot_susceptible<-plotCellTypeProps(
    clusters = susceptible$tosti_cell_type,
...
cell_type_counts_per_sample <- cell_info %>%
  group_by(orig.ident, tosti_cell_type) %>%
  summarise(count = n(), .groups = 'drop') %>%
  pivot_wider(names_from = tosti_cell_type, values_from = count, values_fill = list(count = 0))
...
cell_type_counts_per_sample %>%
  ...
  tab_options(
    ...
    table.font.size = px(10)  # Make the font size smaller
  ) %>%
  cols_align(
    align = "center",
    columns = everything()
  ...
  cols_width(
    orig.ident ~ px(200)  # Make the first column wider
  ) %>%
  fmt_number(
    columns = c(-orig.ident, -protected),
    decimals = 0
  ) %>%
  summary_rows(
    groups = TRUE,
