...
individual_cell_data <- plot_data %>%
    select(protected_status, log_mature_INS, log_nascent_INS) %>%
    pivot_longer(
...
# Prepare data for individual cell plot
individual_cell_data <- plot_data %>%
    select(protected_status, log_mature_INS, log_nascent_INS, orig.ident) %>%
    pivot_longer(
        cols = c(log_mature_INS, log_nascent_INS),
        names_to = "expression_type",
        values_to = "expression"
...
run_mixed_model <- function(data) {
    model <- lmer(expression ~ protected_status + (1 | orig.ident), data = data)
...
- Here p-values were calculated using a linear mixed model with donor identity as a random effect: ***INS expression ~ protected_status + (1 | orig.ident)***. Labels show mean values for each group.
