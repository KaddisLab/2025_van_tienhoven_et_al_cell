...
#' Calculate Gene Signature Scores from a Seurat Object
#'
#'
#' Computes a signature score for each cell by summing expression of provided genes
#' and rescaling to [0,1] range. All genes are treated as positive contributors.
#'
#' @param seurat_object A Seurat object containing single-cell data
#' @param signature_genes Character vector of gene symbols to include in signature
#' @param assay Name of assay to pull expression data from (default: "RNA")
#'
#' @return A numeric vector of signature scores for each cell
#'
#' @examples
#' \dontrun{
#' scores <- calculate_signature_score(seurat_obj, c("CD3E", "CD4", "CD8A"))
#' }
#' @export
calculate_signature_score <- function(
    seurat_object,
    signature_genes,
    assay = "RNA"
) {
    seurat_object <- seurat_object |>
        tidyseurat::join_features(
    ...
    seurat_object |>
        dplyr::mutate(
            pos_score = rowSums(dplyr::across(dplyr::any_of(signature_genes))),
            signature_score = scales::rescale(pos_score, to = c(0, 1))
        ) |>
