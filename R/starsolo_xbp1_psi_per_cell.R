...
#' Calculate XBP1 Splicing Ratio (PSI) Per Cell
#'
#' Calculates the Percent Spliced In (PSI) for XBP1 for each cell using STARsolo
#' splice junction data. The PSI value represents the proportion of spliced XBP1s
#' isoform relative to total XBP1 reads.
#'
#' @param path Path to the STARsolo splice junction Summary.csv file. The function
#'            automatically locates related matrix, barcodes, and features files.
#'
#' @return A tibble containing:
#'   - cell: Cell barcode with sample ID prefix
#'   - xbp1_psi: Percent Spliced In value (0-100)
#'   - xbp1_u: Raw count of reads supporting the critical splicing junction
#'   - xpb1_tot: Total XBP1 reads for the cell
#'
#' @import dplyr Matrix GenomicRanges IRanges S4Vectors
