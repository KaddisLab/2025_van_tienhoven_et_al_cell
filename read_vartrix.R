...
ref <- "/scratch/domeally/DCD.tienhoven_scRNAseq.2024/vartrix_out/HPAP-099_coverage/ref_matrix.mtx"
alt <- sub("ref_matrix", "alt_matrix", ref_path)
barcodes <- sub("ref_matrix.mtx", "colnames.tsv", ref_path)
variants <- read.delim(sub("ref_matrix.mtx", "rownames.tsv", ref_path), header = FALSE, stringsAsFactors = FALSE)[,1]
# read in file to vector


read.delim(sub("ref_matrix.mtx", "rownames.tsv", ref_path), header = FALSE, stringsAsFactors = FALSE)
...
ReadVartrix <- function(genotype, ref, alt, barcodes=c(), tolerance.percent=5, strip.suffix=TRUE){
  ...
  if(!length(variants)){
    warning("The genotype does not contain variants (variants not populated), the row names for the vartrix matrices will not be set.
  ...
  if(dim(vartrix_ref)[1]!=length(variants)){
    warning(paste0("The number of lines in the vartrix reference matrix (",dim(vartrix_ref)[1],") does not match the number of variants in the genotype object (",length(variants),"). Aborting."))
    return(genotype)
  }
  ...
  if(length(variants)) rownames(vartrix_ref) <- variants
  ...
  if(length(variants)) rownames(vartrix_alt) <- variants
  ...
  vartrix <- list(REF=vartrix_ref, ALT=vartrix_alt, FREQ=vartrix_freq_matrix, VAR=vartrix_consensus_matrix)
  return(genotype)
}
